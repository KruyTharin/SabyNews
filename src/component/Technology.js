...
function Technology({ name, color }) {
  const ArrLeft = [
    {
      title:
  ...
  return (
    <div className="relative my-[120px]">
      <Contentitle title="បច្ចេកវិទ្យា" bgcolor="bg-emerald-500"/>
      <div className="-mt-[50px] mb-5 overflow-hidden shadow-lg outline-none">
          ...
          <div className="p-5 md:col-span-2 md:grid lg:col-span-4">
            <div className="flex  grid-cols-2 gap-2  md:grid md:grid-cols-2 lg:grid-cols-4">
              {ArrLeft.map((item, ind) => (
                <RightContent key={ind} image={item.image} title={item.title}/>
              ))}
            </div>
          </div>
        </div>
